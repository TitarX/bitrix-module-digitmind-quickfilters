...
use DigitMind\QuickFilters\Workers\Worker;
...
global $APPLICATION;
$APPLICATION->SetTitle(Loc::getMessage('DIGITMIND_QUICKFILTERS_DOWORK_PAGE_TITLE'));
...
<div class="wrapper">
    <?= Loc::getMessage('DIGITMIND_QUICKFILTERS_DOWORK_PAGE_DESCRIPTION') ?>
</div>
    ...
    <form action="" method="post">
        <div>
            <input type="text" name="selected_file_path" id="selected_file_path" value="<?= $filePath ?>" size="64"
                   placeholder="<?= Loc::getMessage('DIGITMIND_QUICKFILTERS_DOWORK_FILEPATH_PLACEHOLDER_TITLE') ?>"
                   readonly required>
            <button id='open_file_dialog_button' type="button"><?= Loc::getMessage(
                    'DIGITMIND_QUICKFILTERS_DOWORK_FILEPATH_OPEN_TITLE'
                ) ?></button>
        </div>
        <div>
            <input type="submit" value="<?= Loc::getMessage('DIGITMIND_QUICKFILTERS_DOWORK_FORM_SUBMIT') ?>">
        </div>
    </form>
</div>